...
class EntityAI extends Entity
{
	...
	**/
	...
	//! Called when the health gets to the min value, 'oldLevel' is previous health level, 'oldLevel' -1 means this entity was just spawened
	void OnDamageDestroyed(int oldLevel) {};
	
	//! called on server when the entity is destroyed
	...
	//If return true, item can be attached even from paren to this. Item will be switch during proccess. (only hands)
...
#ifdef DEVELOPER
EntityAI _item;//watched item goes here(LCTRL+RMB->Watch)
#endif
