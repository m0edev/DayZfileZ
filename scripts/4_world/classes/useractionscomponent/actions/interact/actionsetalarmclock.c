...
{
	const float TIME_TO_COMPLETE = 12.0;
	...
	override void CreateActionComponent()
	{
		...
		CAContinuousRepeat.Cast(m_ActionData.m_ActionComponent).SetProgress(alarm.m_AlarmTime01 * TIME_TO_COMPLETE);
	}
};
...
class ActionSetAlarmClock: ActionContinuousBase
{
	...
	override void OnStart(ActionData action_data)
	{
		...
		//Print("OnStart " + alarm.m_AlarmTime01);
	}
