class PluginDeveloper extends PluginBase
{
	...
	void HandlePresetSpawn(PlayerBase player, ParamsReadContext ctx)
	{
		...
		{	
			...
			PresetSpawnBase preset_obj;
			
			if (type)
			{
				preset_obj = PresetSpawnBase.Cast(type.Spawn());
			}
			
			if (preset_obj)
			{
				preset_obj.Init(player);
			}
	...
	void SetupSpawnedEntity(EntityAI entity, float health, float quantity = -1, bool special = false)
	{
		if ( special )
			entity.OnDebugSpawn();
